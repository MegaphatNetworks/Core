<#
	...
	even when WinRM is configured and running.  So I decided to use SysInternals PSExec, which is a bit slower but it works.
...
#	$ErrorAction="SilentlyContinue"
	...
	$cnt = 0
	foreach ($ThisComputer in $CompArray) {
		$cnt++
		If (test-connection -computername $ThisComputer -count 1 -quiet){
			$act = "Retrieving users for " + $ThisComputer
